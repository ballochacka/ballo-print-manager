...
export default function CommandesPage() {
  ...
  const totalVente = parseFloat((form.montant || "0").toString().replace(/[^\d.,]/g, "").replace(",", ".")) || 0;
  ...
  const ajouterCommande = async () => {
    ...
    const { error } = await supabase.from("commandes").insert([
      {
        ...
        statut: "En attente",
        montant: form.montant.includes("FCFA") ? form.montant : `${form.montant} FCFA`,
      },
    ]);
    ...
    if (produitStock) {
      await supabase.from("stocks").update({ quantite: produitStock.quantite - quantiteCommandee }).eq("id", produitStock.id);
    }

    setForm({ client: "", produit: "", quantite: "", prix_achat: "", montant: "" });
    setShowForm(false);
  ...
  const imprimerRecu = (cmd: any) => {
    ...
    fenetre.document.write(`
      <html><head><title>Reçu - ${cmd.numero}</title>
      <style>
        body{font-family:Arial;padding:30px;max-width:350px;margin:0 auto}
      ...
      <div class="ligne"><span>N° :</span><span>${cmd.numero}</span></div>
      ...
      <div class="ligne"><span>Qté :</span><span>${cmd.quantite}</span></div>
      <div class="ligne total"><span>Total :</span><span>${cmd.montant}</span></div>
  ...
  const getBenefice = (cmd: any) => {
    const vente = parseFloat((cmd.montant || "0").toString().replace(/[^\d.,]/g, "").replace(",", ".")) || 0;
    return vente - (cmd.prix_achat || 0);
  };
        ...
        {showForm && (
          <div className="mb-6 bg-white rounded-xl border p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select value={form.client} onChange={(e) => setForm({ ...form, client: e.target.value })} className="border rounded-lg px-3 py-2 text-sm">
                <option value="">Sélectionner un client</option>
                {clients.map((c) => <option key={c.id} value={c.nom}>{c.nom}</option>)}
              </select>
              <select value={form.produit} onChange={(e) => setForm({ ...form, produit: e.target.value })} className="border rounded-lg px-3 py-2 text-sm">
                <option value="">Sélectionner un produit</option>
                {stocks.map((s) => <option key={s.id} value={s.nom}>{s.nom} (Stock: {s.quantite})</option>)}
              </select>
              ...
              <input type="number" placeholder="Prix d'achat unitaire (FCFA)" value={form.prix_achat} onChange={(e) => setForm({ ...form, prix_achat: e.target.value })} className="border rounded-lg px-3 py-2 text-sm" />
              <input placeholder="Prix total facturé (FCFA)" value={form.montant} onChange={(e) => setForm({ ...form, montant: e.target.value })} className="border rounded-lg px-3 py-2 text-sm" />
            </div>
            {(form.quantite || form.prix_achat || form.montant) && (
              <div className="mt-4 p-3 bg-purple-50 rounded-lg text-sm space-y-1">
                <p>Coût d'achat : <strong>{totalAchat.toLocaleString()} FCFA</strong></p>
                <p>Prix de vente : <strong>{totalVente.toLocaleString()} FCFA</strong></p>
                <p className="text-green-700 font-bold">Bénéfice : {beneficeEstime.toLocaleString()} FCFA</p>
              </div>
            )}
            <div className="flex gap-3 mt-4">
              <button onClick={ajouterCommande} className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm">Ajouter</button>
                ...
                <tbody className="divide-y">
                  {commandes.map((cmd) => (
                    <tr key={cmd.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-orange-600">{(cmd.prix_achat || 0).toLocaleString()} FCFA</td>
                      <td className="px-4 py-3">{cmd.montant}</td>
                      <td className="px-4 py-3 font-medium text-green-600">{getBenefice(cmd).toLocaleString()} FCFA</td>
                      <td className="px-4 py-3">
                        <select value={cmd.statut} onChange={(e) => changerStatut(cmd.id, e.target.value)} className={`px-2 py-1 rounded-full text-xs border-0 ${getStatutStyle(cmd.statut)}`}>
                      ...
                      <td className="px-4 py-3 flex gap-2">
                        {(cmd.statut === "Prêt" || cmd.statut === "Terminée") && (
                          <button onClick={() => imprimerRecu(cmd)} className="px-2.5 py-1 bg-green-600 text-white text-xs rounded-md">Reçu</button>
                        )}
                        <button onClick={() => supprimerCommande(cmd.id)} className="px-2.5 py-1 bg-red-100 text-red-600 text-xs rounded-md">Supprimer</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
